...
2. **逐层拆解、从大到小(阶段一,理解优先)** - 分析前先带用户把项目拆开,每层都从论文+源码取证:
   - **L0 一句话**:这个项目是干什么的(论文 abstract / README,一句白话)。
   - **L1 顶层组成**:项目分为哪几个部分(典型:任务数据 / 环境 / agent 接口 / 用户模拟 / 评分 / 运行入口),对应论文系统图 + repo 目录结构;讲清每部分的职责与彼此关系。
   - **L2 逐部分展开**:每个部分由哪些模块/类/关键文件组成(文件:行号),做什么、为什么这样设计;用论文/repo 的原始术语讲,不急着套方法论术语。
   - **L3 运行时串讲**:1-2 个真实 case 把各部分协同讲清(先跑哪个文件、进哪个环境、每步输入输出、谁评分、写什么结果)。
   - 对话中**逐层推进、每层确认**:讲完一层问"这层清楚吗?要展开哪块?",用户指哪就钻哪,不一次倾倒整棵树;最终文档里保留完整拆解树。
3. **先解释后提取** - 新手/中级档:每个概念先一句白话(查 `references/glossary.md`)再下结论,多用例子;熟练档可紧凑直接用术语。**降低理解难度优先**:优先用材料原始术语,方法论术语在阶段二引入时必须配白话解释。
