...
> 所有映射的小节标题均已核对 `sources/<id>.md` 实际文本。两份 pypdf 转换的 survey（chang-survey、yehudai-survey）无 Markdown 标题语法，采用源文件中的章节编号 + 实际标题文本；个别标题在 PDF 转换中被空格拆字（如 `3W H A T T O E V A L U A T E`），已注明「(heading garbled in source)」并附行号以便定位。chapter6 中的「过程指标」「Rubric 四准则」等为加粗小标题而非独立 `###` 节，已在映射中标注所属父节。

## A. 评估对象

- LLM 能力 vs agent vs harness 解耦
  - yehudai-survey §7 Discussion > Decoupling LLM & Harness Evaluation（line 842；§2 Agent Capabilities Evaluation 开篇 line 87-88 亦点明 backbone LLM vs agent harness；本主题贯穿全文讨论，非五核心维度之一）
  - chapter6 §引言（`# Agent 的评估` 章首，line 15：评估的对象不应只是模型，而应是模型与 Harness 的组合体；模型替换实验 model swap；区分模型能力不足 vs Harness 设计缺陷）
...
- 评估环境：工具调用型/人机交互型、静态/动态、gym-like
  ...
  - yehudai-survey §5 Core Benchmark Dimensions > Environment（line 501：static vs dynamic environments）+ §6 Frameworks for Agent Evaluation > Gym-like Environments（line 735）
- 评估粒度：最终回答/逐步/轨迹 × 参考有无
  - yehudai-survey §6 Frameworks for Agent Evaluation（line 579；含 Final-response evaluation line 625 / Stepwise Evaluation line 632 / Trajectory-Based Assessment line 657，后者含 Reference-Based line 663 vs Reference-Free line 677）
...
- 质量控制与迭代
  ...
  - chapter12 §12.2.6 扩展与优化建议（line 898；含渐进式评估 gating：5->50->全量、accuracy>0.8 才放量，line 921-928）+ §12.4 数据生成质量评估（line 1832，整体即数据生成的质量验证流程）
...
- LLM-as-judge（偏见：长度/位置/自我偏好...；缓解；多源;jury）
  ...
  - chapter6 §LLM-as-a-Judge：自动化评估的核心（line 281；含同源模型问题与多源异构评判，line 372）
...
- 代码库/实现/加载/prompt/模板/种子差异
  - guidebook §So, you can't reproduce reported model scores?（line 485；含 §DIFFERENT CODE BASE line 489 / §SUBTLE IMPLEMENTATION OR LOADING DIFFERENCE line 499 / §DIFFERENT PROMPT line 533 / Model loading affects reproducibility line 517 / Few-shots samples line 574 / Parameters line 580）
- 归一化、结构化生成
  - guidebook §So, you can't reproduce reported model scores?（line 485，含 Different normalization line 513）+ §Constraining model outputs（line 1472，结构化生成）+ §METRICS 内 Normalization 说明（line 1077，预测文本归一化比较）
